...
public class WishDAO extends AbstractDAO {
    ...
    public static final String SQL_SELECT_MAKING_USER_BY_WISH_ID = "SELECT fulfilled_wish.userId, user_name, surname, photoName" +
    ...
    public static final String SQL_SELECT_USERS_CONSIDERED_BY_WISH_ID = "SELECT considered_wish.userId, user_name, surname, photoName " +
    ...
    public static final String SQL_SELECT_FULFILLED_WISH_BY_OWNER_ID = "SELECT wish.wishId, wish, fulfilled_wish.userId," +
            " user_name, surname, photoName, fulfilled_wish.date FROM wish JOIN fulfilled_wish" +
    ...
    public ArrayList<Wish> getWishes(int userId) throws DAOException {
        ...
        Wish wish;
        try {
            ps = connection.prepareStatement(SQL_SELECT_WISHES_BY_USER_ID);
            ps.setInt(1,userId);
            rs = ps.executeQuery();
            while (rs.next()) {
    ...
    public ArrayList<Wish> getFulfilledWishesByOwnerUserId(int userId) throws DAOException {
        ...
        try {
            ...
            while (rs.next()) {
                wish = new Wish(rs.getInt(1),rs.getString(2));
                wish.setCandidate(new User(rs.getInt(3), rs.getString(4),rs.getString(5),rs.getString(6)));
                wish.setDate(rs.getDate(7));
                wishes.add(wish);
            }
            return wishes;
        }
